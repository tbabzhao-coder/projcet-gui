 ...
 *   PROJECT4_TEST_API_KEY - API key for testing
 *   PROJECT4_TEST_API_URL - API URL (optional)
 *   PROJECT4_TEST_MODEL   - Model to use (optional)
 */
...
test.beforeEach(async ({}, testInfo) => {
  if (!hasApiKey()) {
    testInfo.skip(true, 'Skipping chat tests: PROJECT4_TEST_API_KEY not set')
  }
})

/**
 * Helper to navigate from Home Page to Chat Interface
 * The app shows Home Page first, need to click "进入 Project4" to get to chat
 */
async function navigateToChat(window: any) {
  await window.waitForSelector('#root', { timeout: 10000 })
  await window.waitForLoadState('networkidle')

  // Look for "进入 Project4" text button (the main Project4 temp space card)
  // Try multiple approaches
  let enterProject4 = await window.waitForSelector(
    'text=/进入 Project4/',
    { timeout: 5000 }
  ).catch(() => null)

  if (!enterProject4) {
    // Fallback: look for any element with "Project4" text that's clickable
    enterProject4 = await window.waitForSelector(
      ':text("Project4"):visible',
      { timeout: 5000 }
    ).catch(() => null)
  }

  if (enterProject4) {
    await enterProject4.click()
  }
...
test.describe('Chat Interface', () => {
  test('chat input is visible and functional', async ({ window }) => {
    ...
    await chatInput.fill('Hello, Project4!')
    const value = await chatInput.inputValue()
    expect(value).toBe('Hello, Project4!')
  })
...
test.describe('Real Chat Flow', () => {
  ...
  test('can send message and receive response', async ({ window }) => {
    ...
    // Wait for AI to finish working (wait for "Project4 工作中" to disappear)
    await window.waitForSelector(
      'text="Project4 工作中"',
  ...
  test('displays thinking indicator during response', async ({ window }) => {
    ...
    // Look for working indicator ("Project4 工作中")
    ...
    await window.waitForSelector('text="Project4 工作中"', { state: 'hidden', timeout: 45000 }).catch(() => {})
