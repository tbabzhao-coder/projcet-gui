...
async function navigateToChat(window: any) {
  ...
  // Look for "Enter Project4" or "进入 Project4" text button (supports both EN and CN)
  let enterProject4 = await window.waitForSelector(
    'text=/Enter Project4|进入 Project4/i',
    { timeout: 5000 }
  ).catch(() => null)
...
test.describe('Smoke Tests', () => {
  ...
  test('shows splash or main content', async ({ window }) => {
    ...
    await Promise.race([
      ...
      window.waitForSelector('text=/Project4|API|连接|设置/', { timeout: 5000 }).catch(() => null)
    ])
...
test.describe('Core Features', () => {
  ...
  test('can send message and receive AI response', async ({ window }, testInfo) => {
    ...
    if (!hasApiKey()) {
      testInfo.skip(true, 'Skipping: PROJECT4_TEST_API_KEY not set')
      return
    }
    ...
    await window.waitForSelector('text=/Project4 工作中|Project4 is working/i', { state: 'hidden', timeout: 45000 }).catch(() => {})
