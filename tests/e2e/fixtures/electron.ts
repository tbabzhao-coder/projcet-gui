 ...
 * the Project4 Electron application in E2E tests.
 *
 * Environment Variables:
 *   PROJECT4_TEST_API_KEY   - API key for testing (required for chat tests)
 *   PROJECT4_TEST_API_URL   - API URL (default: https://api.anthropic.com)
 *   PROJECT4_TEST_MODEL     - Model to use (default: claude-haiku-4-5-20251001)
 */
...
const TEST_API_KEY = process.env.PROJECT4_TEST_API_KEY || ''
const TEST_API_URL = process.env.PROJECT4_TEST_API_URL || 'https://api.anthropic.com'
const TEST_MODEL = process.env.PROJECT4_TEST_MODEL || 'claude-haiku-4-5-20251001'
...
function getAppPath(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    const arm64Path = path.join(projectRoot, 'dist/mac-arm64/Project4.app/Contents/MacOS/Project4')
    const x64Path = path.join(projectRoot, 'dist/mac/Project4.app/Contents/MacOS/Project4')
    ...
    const winPath = path.join(projectRoot, 'dist/win-unpacked/Project4.exe')
    if (fs.existsSync(winPath)) {
      return winPath
    }
    throw new Error('Built app not found. Run "npm run build && npm run build:win" first.')
  } else {
    const linuxPath = path.join(projectRoot, 'dist/linux-unpacked/project4')
...
function createTestConfigDir(appPath: string): string {
  ...
  fs.writeFileSync(
    path.join(project4Dir, 'config.json'),
  ...
  // Path: ~/Library/Application Support/Project4/headless-electron/electron-node
  if (process.platform === 'darwin') {
    const userDataDir = path.join(testDir, 'Library', 'Application Support', 'Project4')
