...
describe('Space Service', () => {
  ...
  describe('getTempSpace', () => {
    it('should return the Project4 temp space', () => {
      const tempSpace = getTempSpace()

      expect(tempSpace.id).toBe('project4-temp')
      expect(tempSpace.name).toBe('Project4')
      expect(tempSpace.isTemp).toBe(true)
      expect(tempSpace.icon).toBe('sparkles')
    })

    it('should have valid path', () => {
      const tempSpace = getTempSpace()

      expect(tempSpace.path).toBeTruthy()
      expect(fs.existsSync(tempSpace.path)).toBe(true)
    })

    it('should include stats', () => {
      const tempSpace = getTempSpace()

      expect(tempSpace.stats).toBeDefined()
      expect(typeof tempSpace.stats.artifactCount).toBe('number')
      expect(typeof tempSpace.stats.conversationCount).toBe('number')
    })
  })
  ...
  describe('createSpace', () => {
    ...
    it('should create .project4 directory inside space', async () => {
      const space = await createSpace({
        name: 'Test Space',
        icon: 'folder'
      })

      const project4Dir = path.join(space.path, '.project4')
    ...
    it('should create meta.json with space info', async () => {
      ...
      const metaPath = path.join(space.path, '.project4', 'meta.json')
    ...
    it('should handle custom path', async () => {
      ...
      expect(fs.existsSync(path.join(customPath, '.project4', 'meta.json'))).toBe(true)
    })
  })
  ...
  describe('getSpace', () => {
    ...
    it('should return Project4 space for project4-temp id', () => {
      const space = getSpace('project4-temp')

      expect(space).toBeDefined()
      expect(space?.id).toBe('project4-temp')
      expect(space?.isTemp).toBe(true)
    })
  })

  describe('deleteSpace', () => {
    it('should delete space and its .project4 directory', async () => {
      const space = await createSpace({
        name: 'Delete Test',
        icon: 'folder'
      })
      ...
      // .project4 should be deleted, but space directory may remain (for custom paths)
      expect(fs.existsSync(project4Dir)).toBe(false)
    })

    it('should not allow deleting Project4 temp space', async () => {
      // deleteSpace may return false or throw for temp space
      try {
        const result = await deleteSpace('project4-temp')
