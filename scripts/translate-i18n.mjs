 ...
 *   project4_TEST_API_KEY - API Key
 *   project4_TEST_API_URL - API URL
 *   project4_TEST_MODEL   - Model name
 */
...
const APP_CONTEXT = `
## Product Context
project4 is a desktop AI assistant application (similar to Cursor/Claude Desktop).
...
async function translateBatch(texts, targetLocale) {
  const apiKey = process.env.project4_TEST_API_KEY
  const apiUrl = process.env.project4_TEST_API_URL
  const model = process.env.project4_TEST_MODEL

  if (!apiKey || !apiUrl) {
    throw new Error('Missing project4_TEST_API_KEY or project4_TEST_API_URL in .env.local')
  }
...
3. Keep brand name "project4" untranslated
...
async function main() {
  ...
  console.log(`   API: ${process.env.project4_TEST_API_URL}`)
  console.log(`   Model: ${process.env.project4_TEST_MODEL}\n`)
