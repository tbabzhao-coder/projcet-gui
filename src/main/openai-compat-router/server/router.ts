...
export function createApp(options: RouterOptions = {}): Express {
  ...
  app.post('/v1/messages', async (req: Request, res: Response) => {
    ...
    // Log decoded configuration (for debugging)
    console.log(`[OpenAICompatRouter] ========================================`)
    console.log(`[OpenAICompatRouter] Decoded Configuration from API Key:`)
    console.log(`[OpenAICompatRouter]   Target URL: ${decodedConfig.url}`)
    console.log(`[OpenAICompatRouter]   API Key (first 15 chars): ${decodedConfig.key.substring(0, 15)}...`)
    console.log(`[OpenAICompatRouter]   API Key (last 4 chars): ...${decodedConfig.key.substring(decodedConfig.key.length - 4)}`)
    console.log(`[OpenAICompatRouter]   Model: ${decodedConfig.model || 'not specified'}`)
    console.log(`[OpenAICompatRouter]   API Type: ${decodedConfig.apiType || 'not specified'}`)
    console.log(`[OpenAICompatRouter] ========================================`)
