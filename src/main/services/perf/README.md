...
await window.project4.perfStart(); // Start monitoring
await window.project4.perfStop(); // Stop monitoring
```
...
```javascript
await window.project4.perfStart({
...
```javascript
await window.project4.perfGetState(); // Get current state
await window.project4.perfClearHistory(); // Clear history
await window.project4.perfExport(); // Export as JSON
```
