...
import { getApiCredentials } from './agent'
import {
  isAIBrowserTool,
...
export async function ensureSessionWarm(
  ...
  // Use credentials from getApiCredentials (aiSources.custom) so warm uses same key as sendMessage
  ...
  const sdkOptions: Record<string, any> = {
    ...
    // Skills configuration - pass settingSources if skills are enabled
    ...((() => {
      ...
      // Sync skills to directory (creates/updates .json files)
      const skillsDir = syncSkillsToDirectory(skills)
...
export async function testMcpConnections(mainWindow?: BrowserWindow | null): Promise<{ success: boolean; servers: McpServerStatusInfo[]; error?: string }> {
  ...
  try {
    const config = getConfig()
    let credentials: Awaited<ReturnType<typeof getApiCredentials>>
    try {
      credentials = await getApiCredentials(config)
    } catch (e) {
      return { success: false, servers: [], error: 'API key not configured or invalid' }
    }
    ...
    // Handle OpenAI compatible mode (same as sendMessage) - use credentials from getApiCredentials
    let anthropicBaseUrl = credentials.baseUrl
    let anthropicApiKey = credentials.apiKey
...
export async function sendMessage(
  ...
  // OpenAI compatibility mode: use credentials from getApiCredentials (aiSources.custom)
  let anthropicBaseUrl = credentials.baseUrl
  let anthropicApiKey = credentials.apiKey
  let sdkModel = credentials.model || 'claude-opus-4-5-20251101'
  if (credentials.provider === 'openai') {
...
function getEnabledMcpServers(mcpServers: Record<string, any>): Record<string, any> | null {
  ...
  for (const [name, config] of Object.entries(mcpServers)) {
    if (!config.disabled) {
      // Remove the 'disabled' and '__builtIn' fields before passing to SDK (Project4 extensions)
      const { disabled, __builtIn, ...sdkConfig } = config as any
...
// Get enabled skills (filter out disabled)
function getEnabledSkills(skills: Record<string, any>): Record<string, any> | null {
  if (!skills || Object.keys(skills).length === 0) {
    return null
  }
  ...
  for (const [name, config] of Object.entries(skills)) {
    if (!config.disabled) {
      const { disabled, importedAt, source, ...skillData } = config as any
      enabled[name] = skillData
    }
  }
...
function ensureSkillsDirectory(): string {
...
// Sync skills from config to directory (write enabled skills as .json files)
function syncSkillsToDirectory(skills: Record<string, any>): string {
  const skillsDir = ensureSkillsDirectory()

  // Get enabled skills
  const enabledSkills = getEnabledSkills(skills)
  if (!enabledSkills) {
    // Clear all skill files if no enabled skills
    const existingFiles = readdirSync(skillsDir).filter(f => f.endsWith('.json'))
  ...
  // Write/update enabled skills
  const currentSkills = new Set<string>()
  for (const [name, skill] of Object.entries(enabledSkills)) {
    currentSkills.add(name)
    const filePath = join(skillsDir, `${name}.json`)

    // Write in Claude Code CLI compatible format
    const skillContent = {
      name: skill.name,
    ...
    writeFileSync(filePath, JSON.stringify(skillContent, null, 2), 'utf-8')
  }

  // Remove skill files that are no longer enabled
  for (const existingName of existingFiles) {
    if (!currentSkills.has(existingName)) {
      unlinkSync(join(skillsDir, `${existingName}.json`))
    }
  }
...
function calculateSkillsHash(skills: Record<string, any> | undefined): string {
  if (!skills) return ''

  const enabledSkills = getEnabledSkills(skills)
  if (!enabledSkills) return ''

  // Create a stable string representation of enabled skills
  const skillNames = Object.keys(enabledSkills).sort()
