...
import { getTempSpacePath, getProject4Dir } from './config.service'
...
export class SearchService {
  ...
  private async searchFile(filePath: string, query: string): Promise<SearchResult[]> {
    ...
    try {
      ...
      // Get space name
      let spaceName = data.spaceId === 'project4-temp' ? 'Project4' : data.spaceId

      try {
        if (data.spaceId !== 'project4-temp') {
  ...
  private getFilesToSearch(
    ...
    if (scope === 'space' && spaceId) {
      ...
      if (spaceId === 'project4-temp') {
        ...
        try {
          ...
          if (space) {
            return this.scanConversationFiles(join(space.path, '.project4', 'conversations'))
          }
        } catch (e) {
    ...
    if (scope === 'global') {
      ...
      const spacesDir = join(project4Dir, 'spaces')
      if (existsSync(spacesDir)) {
        const spaceNames = readdirSync(spacesDir)
        spaceNames.forEach(spaceName => {
          try {
            const convDir = join(spacesDir, spaceName, '.project4', 'conversations')
  ...
  private findConversationFile(conversationId: string, spaceId?: string): string | null {
    ...
    if (spaceId) {
      if (spaceId === 'project4-temp') {
        ...
        try {
          const space = getSpace(spaceId)
          if (space) {
            const filePath = join(space.path, '.project4', 'conversations', `${conversationId}.json`)
            if (existsSync(filePath)) {
              return filePath
            }
          }
    ...
    if (existsSync(spacesDir)) {
      ...
      for (const spaceName of spaceNames) {
        try {
          filePath = join(spacesDir, spaceName, '.project4', 'conversations', `${conversationId}.json`)
          if (existsSync(filePath)) {
            return filePath
          }
        } catch (e) {
