 ...
 * enabling true browser functionality within Project4 - like having Chrome embedded
 * in the app.
 *
 * Key features:
...
class BrowserViewManager {
  ...
  async create(viewId: string, url?: string): Promise<BrowserViewState> {
    ...
    const view = new BrowserView({
      webPreferences: {
        ...
        // NOTE: webSecurity: true may block SSO redirects and cross-origin requests
        // If SSO redirects fail (white screen), consider setting to false
        // However, this reduces security, so use with caution
        webSecurity: false,
        // Allow insecure content (HTTP) for internal networks
        // Many internal networks use HTTP or self-signed certificates
        allowRunningInsecureContent: true,
        // Persistent storage for cookies, localStorage, etc.
        // This ensures cookies are preserved across SSO redirects
        partition: 'persist:browser',
        // Enable smooth scrolling and other web features
        scrollBounce: true,
        // Additional settings for better SSO/redirect support
        // Allow navigation to data URLs (some SSO flows use these)
        // Note: This is already the default, but explicit for clarity
      },
    })
    ...
    if (url) {
      try {
        ...
        // Use extraHeaders to ensure cookies are sent with redirects
  ...
  async navigate(viewId: string, input: string): Promise<boolean> {
    ...
    if (!url.startsWith('http://') && !url.startsWith('https://') && !url.startsWith('file://')) {
      // Check if it's an IP address (for internal networks)
      if (this.isIPAddress(url)) {
        url = 'http://' + url
      }
      // Check if it looks like a domain
      else if (url.includes('.') && !url.includes(' ') && this.looksLikeDomain(url)) {
        url = 'https://' + url
      } else {
    ...
    try {
      // Use extraHeaders to ensure cookies are sent with redirects (important for SSO)
      await view.webContents.loadURL(url, {
        extraHeaders: 'Cache-Control: no-cache\n',
      })
      return true
  ...
  /**
   * Check if input is an IP address (for internal networks)
   */
  private isIPAddress(input: string): boolean {
    // IPv4 pattern: 192.168.1.1, 10.0.0.1, etc.
    const ipv4Pattern = /^(\d{1,3}\.){3}\d{1,3}(:\d+)?$/
    // IPv6 pattern: [::1], [2001:db8::1], etc.
    ...
    if (ipv4Pattern.test(input)) {
      ...
      const octets = withoutPort.split('.')
  ...
  destroyAll() {
    ...
    const timers = Array.from(this.stateChangeDebounceTimers.values())
  ...
  private bindEvents(viewId: string, view: BrowserView) {
    ...
    wc.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
      ...
      // Log detailed error information for debugging SSO/redirect issues
      console.error(`[BrowserView] Navigation failed for view ${viewId}:`, {
        errorCode,
        errorDescription,
        validatedURL,
        ...
        isMainFrame,
        // Common error codes:
        // -2: ERR_FAILED (generic failure)
        // -6: ERR_FILE_NOT_FOUND
        // -7: ERR_TIMED_OUT
        // -20: ERR_BLOCKED_BY_CLIENT (ad blocker, security policy)
        // -21: ERR_NETWORK_CHANGED
        // -106: ERR_INVALID_RESPONSE
        // -118: ERR_CONNECTION_TIMED_OUT
        // -200: ERR_CERT_AUTHORITY_INVALID
        // -201: ERR_CERT_DATE_INVALID
        // -202: ERR_CERT_COMMON_NAME_INVALID
      })

      // For SSO redirect issues, provide more helpful error messages
      let userFriendlyError = errorDescription || `Error ${errorCode}`
      if (errorCode === -20) {
        userFriendlyError = '页面被安全策略阻止（可能是跨域或 SSO 跳转问题）'
      } else if (errorCode === -2 && validatedURL && validatedURL.includes('sso')) {
        userFriendlyError = 'SSO 跳转失败，可能是跨域安全策略阻止'
      }
    ...
    wc.on('will-navigate', (event, url) => {
      ...
      // Also allow about:blank for SSO redirects that may use it
      if (
        !url.startsWith('http://') &&
        ...
        console.log(`[BrowserView] Blocked navigation to non-http(s) URL: ${url}`)
        event.preventDefault()
      } else {
        console.log(`[BrowserView] Allowing navigation to: ${url}`)
      }
    })

    // Additional event handlers for better SSO/redirect debugging
    wc.on('did-redirect-navigation', (event, url, isInPlace, isMainFrame) => {
      if (isMainFrame) {
        console.log(`[BrowserView] Redirect detected: ${url} (in-place: ${isInPlace})`)
        // Update URL immediately on redirect to show progress
        this.updateState(viewId, {
          url,
          isLoading: true,
        })
    ...
    // Log console errors for debugging SSO issues
    wc.on('console-message', (event, level, message, line, sourceId) => {
      if (level >= 2) { // Error or warning
        console.log(`[BrowserView] Console ${level === 2 ? 'warning' : 'error'}: ${message} (${sourceId}:${line})`)
      }
    })

    // Handle SSL certificate errors (common for internal networks with self-signed certificates)
    // This allows access to internal websites that use self-signed or invalid certificates
    wc.on('certificate-error', (event, url, error, certificate, callback) => {
      console.log(`[BrowserView] Certificate error for ${url}:`, {
        error,
      ...
      // For internal networks, allow self-signed certificates
      ...
      if (isInternalNetwork) {
        console.log(`[BrowserView] Allowing certificate error for internal network: ${url}`)
        event.preventDefault()
        callback(true) // Allow the certificate
      } else {
        // For external sites, use default behavior (reject invalid certificates)
        console.log(`[BrowserView] Rejecting certificate error for external site: ${url}`)
