...
export function registerApiRoutes(app: Express, mainWindow: BrowserWindow | null): void {
  ...
  // ===== Space Routes =====
  app.get('/api/spaces/project4', async (req: Request, res: Response) => {
    const result = spaceController.getTempSpaceController()
    res.json(result)
  })
  ...
  // TODO: Re-implement loadTreeChildren or remove this route
  ...
  app.get('/api/spaces/:spaceId/artifacts/download-all', async (req: Request, res: Response) => {
    try {
      ...
      const fileName = spaceId === 'project4-temp' ? 'project4-artifacts' : basename(workDir)
      res.setHeader('Content-Type', 'application/gzip')
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}.tar.gz"`)
