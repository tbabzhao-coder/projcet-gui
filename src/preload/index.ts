...
const api: Project4API = {
  ...
  selectFolder: () => ipcRenderer.invoke('space:select-folder'),
  selectFile: (options) => ipcRenderer.invoke('space:select-file', options),
...
// This is used by the overlay window which doesn't need the full project4 API
const electronAPI = {
  ipcRenderer: {
...
// TypeScript declaration for window.project4 and window.platform
declare global {
  interface Window {
    project4: Project4API
    platform: {
