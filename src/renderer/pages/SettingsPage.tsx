...
export function SettingsPage() {
  ...
  const autoSave = useCallback(async (partialConfig: Partial<AppConfig>) => {
    const newConfig = { ...config, ...partialConfig } as AppConfig
  ...
  const handleThemeChange = async (value: ThemeMode) => {
    ...
    try {
      localStorage.setItem('project4-theme', value)
    } catch (e) { /* ignore */ }
    await autoSave({
  ...
  const handleMcpServersSave = async (servers: McpServersConfig) => {
    ...
    setConfig({ ...config, mcpServers: servers } as AppConfig)
  }

  // Handle Skills save
  const handleSkillsSave = async (skills: SkillsConfig) => {
    await api.setConfig({ skills })
        ...
        <div className="max-w-2xl mx-auto space-y-8">
                ...
                .map((provider) => {
                  ...
                  if (isLoggedIn) {
                    ...
                    return (
                      <div
                        key={provider.type}
                        className={`p-5 rounded-xl border-2 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md ${currentSource === provider.type
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            {/* Provider Icon */}
                            <div
                              className="w-12 h-12 rounded-xl flex items-center justify-center transition-transform duration-200 hover:scale-110"
                          ...
                          <button
                            ...
                            className="p-2 text-foreground-secondary hover:text-destructive hover:bg-destructive/10 rounded-lg active:scale-95 transition-all duration-200"
                            title={t('Logout')}
                          >
                            {isLoggingOut ? (
                    ...
                    return (
                      <button
                        ...
                        className="w-full p-5 rounded-xl border-2 border-dashed border-border hover:border-primary/50 hover:bg-primary/5 active:scale-[0.98] transition-all duration-200 flex items-center gap-3"
                      >
                        <div
                          ...
                          <div className="space-y-4 pt-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                placeholder={provider === 'openai' ? "gpt-4o-mini" : "claude-sonnet-4-5-20250929"}
                                className="w-full px-4 py-2 text-sm bg-input rounded-lg border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none transition-all duration-200"
                              />
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                              <button
                                ...
                                className="px-4 py-2 text-sm bg-primary text-white font-medium rounded-lg shadow-sm hover:shadow-md hover:bg-primary-hover active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
          ...
          {/* <section className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="bg-muted/50 rounded-lg p-3 mb-4 text-sm text-muted-foreground">
              {t('We recommend full trust mode - use natural language to control Project4. UI-based permission settings coming in future versions.')}
            </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        title={t('Automatically run Project4 when system starts')}
                      >
                        ?
                      </span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {t('Automatically run Project4 when system starts')}
                    </p>
                  </div>
          ...
          <section className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="mt-4 pt-4 border-t border-border">
              {/* <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-muted-foreground">{t('Format compatible with Claude Code CLI')}</span>
              </div> */}
              {/* <p className="text-xs text-amber-500/80">
                ⚠️ {t('Configuration changes will take effect after starting a new conversation')}
              </p> */}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t('Allow access to Project4 from other devices')}
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
