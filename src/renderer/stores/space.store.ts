...
export const useSpaceStore = create<SpaceState>((set, get) => ({
  ...
  loadProject4Space: async () => {
    try {
      ...
      console.error('Failed to load Project4 space:', error)
    }
  },
  ...
  updateSpacePreferences: async (spaceId, preferences) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Update in spaces list or project4 space
        if (updatedSpace.isTemp) {
          set({ project4Space: updatedSpace })
        } else {
          set((state) => ({
            spaces: state.spaces.map((s) =>
              s.id === spaceId ? updatedSpace : s
  ...
  getSpacePreferences: (spaceId) => {
    const { project4Space, spaces, currentSpace } = get()
    ...
    // Check project4 space
    if (project4Space?.id === spaceId) {
      return project4Space.preferences
    }
