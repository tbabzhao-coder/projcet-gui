 ...
 *
 * Flow:
 * 1. step 1: Highlight Project4 Space card on HomePage
...
export type OnboardingStep = 'project4-space' | 'send-message' | 'view-artifact' | 'completed'
...
export const useOnboardingStore = create<OnboardingState>((set, get) => ({
  ...
  startOnboarding: () => {
    set({ isActive: true, currentStep: 'project4-space' })
  },

  nextStep: () => {
    const { currentStep } = get()

    const stepOrder: OnboardingStep[] = ['project4-space', 'send-message', 'view-artifact', 'completed']
