...
// Detect if running in Electron (has window.project4 via preload)
export function isElectron(): boolean {
  return typeof window !== 'undefined' && 'project4' in window
}
...
export function getAuthToken(): string | null {
  if (typeof localStorage !== 'undefined') {
    return localStorage.getItem('project4_remote_token')
  }
  return null
}
...
export function setAuthToken(token: string): void {
  if (typeof localStorage !== 'undefined') {
    localStorage.setItem('project4_remote_token', token)
  }
}
...
export function clearAuthToken(): void {
  if (typeof localStorage !== 'undefined') {
    localStorage.removeItem('project4_remote_token')
  }
}
...
export async function httpRequest<T>(
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      document.cookie = 'project4_authenticated=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'
...
export function onEvent(channel: string, callback: (data: unknown) => void): () => void {
  if (isElectron()) {
    // Use IPC in Electron
    const methodMap: Record<string, keyof typeof window.project4> = {
      ...
      'debug:api-log': 'onDebugApiLog'
    }

    const method = methodMap[channel]
    if (method && typeof window.project4[method] === 'function') {
      return (window.project4[method] as (cb: (data: unknown) => void) => () => void)(callback)
    }
