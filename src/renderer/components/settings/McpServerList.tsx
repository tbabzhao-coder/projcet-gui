      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {/* Built-in badge */}
            {(config as any).__builtIn && (
              <span className="px-1.5 py-0.5 text-xs bg-primary/10 text-primary rounded border border-primary/20 whitespace-nowrap flex-shrink-0">
                {t('Built-in')}
              </span>
            )}
            {isDisabled && (
              ...
              <div className="space-y-4">
                ...
                {serverType === 'stdio' && 'command' in editingConfig && (
                  <>
                    ...
                    {/* Playwright-specific: Browser Args */}
                    {/* Show browser args editor for Playwright MCP (check by name or by checking if args contains playwright/mcp) */}
                    {(name === 'playwright' || 
                      ((editingConfig as any).args || []).some((arg: string) => 
                        typeof arg === 'string' && arg.includes('@playwright/mcp')
                      )) && (
                      <div className="mt-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                        <label className="block text-sm font-medium text-foreground mb-2">
                          {t('Browser Launch Arguments')} <span className="text-xs text-muted-foreground">(Playwright MCP)</span>
                        </label>
                        <p className="text-xs text-muted-foreground mb-3">
                          {t('Configure browser launch options to bypass security restrictions. Example: --disable-web-security')}
                        </p>
                        <div className="space-y-2">
                          {((editingConfig as any).browserArgs || []).map((arg: string, index: number) => (
                            <div key={index} className="flex items-center gap-2">
                              <input
                                type="text"
                                value={arg}
                                onChange={e => {
                                  const browserArgs = [...((editingConfig as any).browserArgs || [])]
                                  browserArgs[index] = e.target.value
                                  updateVisualField('browserArgs', browserArgs)
                                }}
                                className="flex-1 px-3 py-2 border border-border rounded-lg bg-input text-foreground focus:ring-2 focus:ring-primary focus:border-transparent font-mono text-sm transition-colors"
                              ...
                              <button
                                onClick={() => {
                                  const browserArgs = ((editingConfig as any).browserArgs || []).filter((_: string, i: number) => i !== index)
                                  updateVisualField('browserArgs', browserArgs)
                                }}
                                className="p-2 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded transition-colors"
                              >
                          ...
                          >
                            <Plus className="w-4 h-4" />
                            {t('Add browser argument')}
                          </button>
                        </div>
                        <div className="mt-3 pt-3 border-t border-amber-500/20">
                          <p className="text-xs text-amber-600 dark:text-amber-400">
                            💡 {t('Common options:')} <code className="bg-amber-500/20 px-1 rounded">--disable-web-security</code>, <code className="bg-amber-500/20 px-1 rounded">--disable-features=IsolateOrigins,site-per-process</code>
                          </p>
                        </div>
                      </div>
                    )}
