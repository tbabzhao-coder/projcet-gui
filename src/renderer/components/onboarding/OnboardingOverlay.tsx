...
const STEP_CONFIGS = {
  'project4-space': {
    targetSelector: '[data-onboarding="project4-space"]',
    tooltipPosition: 'bottom' as const,
    titleKey: 'Start your first conversation',
    descriptionKey: 'Click to enter Project4 space and experience how AI can help you',
    icon: Sparkles,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 mt-4 pt-3 border-t border-border">
        {['project4-space', 'send-message', 'view-artifact'].map((step, index) => (
          <div
            key={step}
            className={`w-2 h-2 rounded-full transition-colors ${
              step === currentStep
                ? 'bg-primary'
                : index < ['project4-space', 'send-message', 'view-artifact'].indexOf(currentStep)
                ? 'bg-primary/50'
                : 'bg-muted'
            }`}
          />
        ...
        <span className="text-xs text-muted-foreground ml-2">
          {['project4-space', 'send-message', 'view-artifact'].indexOf(currentStep) + 1} / 3
        </span>
      </div>
    </Spotlight>
  )
