 ...
 * - AI response text
 * - HTML artifact content (a beautiful PPT introducing Project4)
 *
...
export const ONBOARDING_ARTIFACT_NAME = 'project4-intro.html'
...
export function getOnboardingPrompt(translator?: TFunction): string {
  ...
  return t(
    'Help me create an HTML slide deck introducing Project4 (1920x1080) with a clean, modern style. Include: 1) What is Project4 2) Core capabilities 3) Use cases 4) Getting started'
  )
}
...
export function getOnboardingAiResponse(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t(
    "Sure, I'll create a polished HTML presentation that introduces Project4's core features and how to use it. The slides use a modern, minimal design, support left/right arrow navigation, and fit 1920x1080."
  )
}

export function getOnboardingConversationTitle(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t('Welcome to Project4')
}

export function getOnboardingHtmlArtifact(translator?: TFunction, lang: string = getCurrentLanguage()): string {
  const t = getTranslator(translator)

  const docTitle = t('Project4 - AI that gets things done')
  const tagline = t('AI that gets things done')
  const whatIsProject4 = t('What is Project4?')
  const moreThanChatTitle = t('More than chat')
  const moreThanChatDesc = t('Project4 understands your goals, plans tasks, and can execute autonomously—not just answer questions.')
  ...
  const naturalLangDesc = t('Describe what you want in plain language; Project4 will understand and act.')
  ...
  const writeCodeDesc = t('Describe your needs and Project4 will generate complete code across languages and frameworks.')
  ...
  const webDesc = t('Need fresh information? Project4 can search online and summarize what matters.')
  const docsTitle = t('Write documents')
  const docsDesc = t('Reports, proposals, emails—tell Project4 what you need and it will draft it.')
  ...
  const startText = t('Start using Project4')
...
<body>
  <div class="slides">
    ...
    <!-- Slide 2: What is Project4 -->
    <section class="slide slide-content">
      <h2>${whatIsProject4}</h2>
      <div class="features">
        <div class="feature">
