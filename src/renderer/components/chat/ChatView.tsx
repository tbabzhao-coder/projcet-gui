...
export function ChatView({ isCompact = false }: ChatViewProps) {
  ...
  // Handle send (with optional images for multi-modal messages, optional thinking mode, optional plan mode)
  const handleSend = async (content: string, images?: ImageAttachment[], thinkingEnabled?: boolean, planModeEnabled?: boolean) => {
    ...
    // Pass AI Browser, thinking, and plan mode state to sendMessage
...
function EmptyState({ isTemp, isCompact = false }: { isTemp: boolean; isCompact?: boolean }) {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center px-8">
      {/* Icon - tech robot with subtle glow */}
      <div className="relative flex items-center justify-center">
        <div className="absolute w-14 h-14 rounded-full bg-primary/10 blur-sm" />
        <Bot className="relative w-12 h-12 text-primary drop-shadow-sm" strokeWidth={1.5} />
      </div>
