...
export function Header({ left, right, className = '' }: HeaderProps) {
  ...
  // In Electron frameless window: whole header is draggable; left/right keep no-drag so buttons work
  const headerDragClass = isInElectron ? 'drag-region' : ''
  ...
  return (
    <header
      ...
      `.trim().replace(/\s+/g, ' ')}
    >
      <div className="flex items-center gap-2 sm:gap-3 no-drag min-w-0">
        {left}
      </div>

      {/* Spacer between left and right (also draggable when header has drag-region) */}
      <div className={`flex-1 ${isInElectron ? 'drag-region' : ''}`} />
