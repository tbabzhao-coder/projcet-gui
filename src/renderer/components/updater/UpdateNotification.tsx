...
export function UpdateNotification() {
  ...
  const handleInstall = () => {
    if (isManualDownload || isMac) {
      // Open release page for manual download (macOS always, or when manual-download status)
      if (notificationVersion) {
        // TODO: Replace with your own update server or website
        window.open(
          `https://your-website.com/releases/v${notificationVersion}`,
          '_blank'
        )
      }
    } else {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-sm font-medium text-zinc-100">
              {t('New version Project4 {{version}} available', { version: notificationVersion })}
            </h4>
