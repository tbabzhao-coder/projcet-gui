...
class CanvasLifecycle {
   ...
   * Note: BrowserView can access file:// directly, no need for project4-file://
   */
  ...
  private async loadFileContent(tabId: string, path: string, type: ContentType): Promise<void> {
    ...
    // Images use project4-file:// protocol directly (no content loading needed)
    if (type === 'image') {
      tab.isLoading = false
      this.notifyTabsChange()
      return
